...
# --- 6. INJEKSI CSS ---
# Bagian 1: CSS Statis (Tanpa 'f' agar aman dari error kurung kurawal)
st.markdown("""
    <style>
...
# Bagian 2: CSS Dinamis (Menangani background dinamis secara terpisah dan aman)
st.markdown(f"""
    <style>
    .stApp {{
...
# --- MODUL TERBARU K3 LAB (LINK TERHUBUNG) ---
elif menu == "🛡️ K3 & Keamanan Lab":
    st.markdown("<h1 style='text-align: center;'>🛡️ Portal Keselamatan Lab (K3)</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #a8b2d1;'>Pelajari panduan keselamatan standar, pengenalan APD, simbol GHS, dan simulasi keadaan darurat.</p>", unsafe_allow_html=True)
    st.write("")

    col_mid = st.columns([1, 2, 1])[1]
    with col_mid:
        with st.container(border=True):
            st.markdown("<h3 style='text-align: center; color: #00e676;'>Masuk ke Modul K3 Interaktif</h3>", unsafe_allow_html=True)
            st.caption("Dilengkapi dengan Aturan Emas, Prosedur Darurat, dan Mini Kuis Evaluasi Kelayakan Masuk Lab.")
            st.write("")
            st.link_button("Buka Modul K3 Laboratorium 🚀", "https://k3lab2aadpgqh9jp2gqe5brxeab.streamlit.app/", use_container_width=True)
    ...
    if prompt := st.chat_input("Ketik di sini (contoh: tolong carikan corong pisah)"):
        ...
        if not ketemu:
            if any(k in prompt_l for k in ["bahaya", "k3", "aman", "jas lab"]):
                res_txt = "🛡️ **Keamanan adalah prioritas!** Buka menu Panduan Keamanan Lab untuk info lebih detail."
